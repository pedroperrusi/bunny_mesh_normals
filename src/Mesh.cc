...
namespace bunny_mesh
{
    ...
    void TriangleMesh::ComputeFaceNormals()
    {
        ...
        for(size_t i = 0; i < this->num_faces; i++)
        {
            ...
            Eigen::Vector3d v0 = this->vertices.row(verticesIdx.coeff(0));
            Eigen::Vector3d v1 = this->vertices.row(verticesIdx.coeff(1));
            Eigen::Vector3d v2 = this->vertices.row(verticesIdx.coeff(2));
